...
class TestFindDuplicates:
    ...
    def test_find_duplicates_same_filename_different_paths(self):
        ...
        assert (
            duplicates["IMG_001"]["keep"]["public_id"]
            == "myblog/travelogue/camino/ch1/IMG_001"
        )
...
class TestUploadFile:
    ...
    def test_upload_file_success_image(
        self, mocker, tmp_path, mock_cloudinary_upload_response
    ):
        ...
        content_dir.mkdir()
        (content_dir / "test_image.jpg").write_bytes(b"fake image data")

        original_cwd = Path.cwd()
        try:
            os.chdir(tmp_path)
            # upload_file derives the public_id from the path relative to content/
            result = upload_file(Path("content/test_image.jpg"), folder="myblog")
    ...
    @pytest.mark.unit
    @pytest.mark.cloudinary
    def test_upload_file_success_video(
        self, mocker, tmp_path, mock_cloudinary_video_upload_response
    ):
        ...
        try:
            os.chdir(tmp_path)
            # upload_file derives the public_id from the path relative to content/
            result = upload_file(Path("content/test_video.mp4"), folder="myblog")
...
class TestLoadExistingMapping:
    ...
    def test_load_existing_mapping_backward_compatibility(self, tmp_path):
        ...
        assert isinstance(
            mapping["travelogue/camino/ch1/IMG_001.jpg"], CloudinaryResource
        )
...
class TestUpdateMarkdownFile:
    ...
    def test_update_markdown_file_success(self, tmp_path):
        ...
        md_file.write_text(
            "![Alt text](IMG_001.jpg)\n![Another](IMG_002.png)", encoding="utf-8"
        )
    ...
    def test_update_markdown_file_partial_matches(self, tmp_path):
        ...
        md_file = tmp_path / "test.md"
        md_file.write_text(
            "![Alt](IMG_001.jpg)\n![Another](NOT_IN_MAPPING.jpg)", encoding="utf-8"
        )
...
class TestCompressVideo:
    ...
    @pytest.mark.unit
    @pytest.mark.ffmpeg
    def test_compress_video_ffmpeg_not_found(
        self, mocker, tmp_path, mock_ffmpeg_not_found
    ):
