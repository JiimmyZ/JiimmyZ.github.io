...
### summary 的本質：列表頁上讀者看得到的內容

summary 不是一段附加的介紹文字，它就是**首頁列表、標籤頁這些地方讀者不點進文章就會看到的東西**。想清楚這點，短篇和長篇的寫法差異就不是兩條各自獨立的規則，而是同一個邏輯的兩種結果：

- **詩詞、雜感夠短，短到沒必要讓讀者點進文章頁才看得到內文**——所以乾脆讓 summary＝內文，列表頁直接顯示全文。這是**預設做法，不是特例**。例如：
  ...
  只有目前站上還沒出現過的「長詩」才不適用——如果哪天 JZ 寫了一首長到不適合攤在列表頁的詩，那就要跟長篇一樣手寫總結，不能再套這條預設。

- **長篇散文和遊記反過來**：全文本來就不會（也不該）攤在列表頁給人看，所以 summary 才需要是真的手寫濃縮（例如遊記 ch3 的 `"通勤、住宿選擇與實用網站、App整理"`）。長篇一律請 JZ 自己給，不要用內文湊。

即使是預設做法，也還是要問過再寫——這條規則本身沒有變。短篇可以這樣問：「summary 我照慣例幫你帶成『快樂不值得追求 痛苦不值得畏懼』，可以嗎？」——他點頭你才寫進去。
