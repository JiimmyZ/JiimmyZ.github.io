...
**問的時機：建檔之前一次問完，等他回答再動手。** 不要一邊問一邊把檔案建掉——那等於沒問。

這個站只有這一道關卡。三個答案到手之後，後面建檔、commit、開 PR、合併全部一路做完，不要再回頭確認「這樣可以嗎」。JZ 要的是「開頭問清楚，然後放手做完」，不是每一步都來報備。
...
- **`date` 用發布當下的台北時間**。容器預設是 UTC，要指定時區：
  ...
  ```
- **`draft = false`**——寫 true 的話網站上看不到。唯一的例外是**骨架稿**：JZ 說「先把架構弄好、正文等等給你」的時候，正文還是空的，這種先寫 `draft = true`，等內容補齊再改回 false。這樣半成品不會因為自動合併就上線。記得在交回去時提醒他這件事。
...
寫完一定要驗，這行會把漏掉兩空格的行直接印出來——**沒有輸出就是通過**：

```bash
sed '1s/^\xEF\xBB\xBF//' content/poetry/標題.md \
  | awk '/^\+\+\+$/{n++; next} n>=2 && NF && !/^自註/ && !/  $/ {print NR": 缺兩空格 → "$0}'
```

（開頭那段 `sed` 是在剝 BOM。舊檔有 13 個帶 BOM，不剝的話 `+++` 比對不到，檢查會靜靜跳過整個正文然後回報「通過」——這種假通過比不檢查更危險。）
