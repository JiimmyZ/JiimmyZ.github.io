...
### Session Log 2026-08-25 - 釐清 summary 的判斷邏輯

**Summary**: JZ 發現自己已經忘記詩詞的 summary 為什麼跟內文一模一樣，藉這次機會把邏輯講清楚並記錄下來，避免以後又要重新推理一次。

**釐清的邏輯**：summary 的本質是「首頁列表、標籤頁這些地方，讀者不點進文章就會看到的內容」。

- 詩詞、雜感夠短，短到沒必要讓讀者點進去才看得到內文，所以乾脆讓 summary＝內文（換行改空格），列表頁直接顯示全文。這是**預設做法，不是特例**——只有以後真的寫了「長詩」（目前站上還沒有）才不適用。
- 長篇小說、長篇遊記則相反：全文本來就不會攤在列表頁給人看，所以才需要真正手寫總結。

已同步更新 `.claude/skills/publish-post/SKILL.md` 的對應段落，把原本標成「特例」的措辭改成「預設做法＋為什麼」；「summary 一律要問過 JZ 才寫」這條規則本身沒變。
