"""
Fix duplicate path segments in Cloudinary file paths for ch7.

Cloudinary files have paths like:
  myblog/travelogue/camino/ch7/myblog/travelogue/camino/ch7/IMG_xxx

Should be:
  myblog/travelogue/camino/ch7/IMG_xxx

This script uses Cloudinary API to rename/move files to correct paths.
Based on fix_cloudinary_paths.py but specifically for ch7.
"""
...
def fix_path(public_id):
    ...
    # Look for pattern: myblog/travelogue/camino/ch7/myblog/...
    if len(parts) < 5:
...
def fix_all_paths(resource_type="image", folder="myblog/travelogue/camino/ch7", dry_run=True):
...
if __name__ == "__main__":
    ...
    # Fix images
    print("Fixing ch7 images...")
    img_fixed, img_skipped, img_errors = fix_all_paths("image", "myblog/travelogue/camino/ch7", dry_run)
    
    # Fix videos
    print("\n\nFixing ch7 videos...")
    vid_fixed, vid_skipped, vid_errors = fix_all_paths("video", "myblog/travelogue/camino/ch7", dry_run)
